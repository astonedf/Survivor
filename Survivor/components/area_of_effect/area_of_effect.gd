...
signal affectable_area_entered(aoe: AreaOfEffect, affectable_area: AffectableArea)
signal damage_done(to: Node2D, amount: int)
signal heal_done(to: Node2D, amount: int)
...
func _on_area_entered(area: Area2D) -> void:
	affectable_area_entered.emit(self, area)
